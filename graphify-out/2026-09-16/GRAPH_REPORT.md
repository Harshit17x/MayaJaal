...
## Summary
- 1373 nodes · 2320 edges · 79 communities (60 shown, 9 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 49 edges (avg confidence: 0.94)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `05096a76`
...
5. `Postprocessor` - 22 edges
6. `ONNXEngine` - 21 edges
7. `Preprocessor` - 21 edges
...
Cohesion: 0.12
Nodes (14): ByteTracker configuration. Maps to the supervision ByteTrack constructor…, Immutable configuration for ByteTrackerWrapper. Attributes:…, TrackerConfig, ByteTrack multi-object tracking package. Provides stateful, per-session object…, Thread-safe store for per-camera ByteTracker sessions. Each RTSP camera is…, Metadata wrapper around a tracker session., Return the existing tracker for ``camera_id``, or create a new one. Args:…, SessionInfo (+6 more)
...
### Community 3 - "api.ts"
Cohesion: 0.07
Nodes (37): COLOR_PRESETS, FeedViewMode, ToolMode, ApiError, request(), StreamValidationResult, InferenceResponse, ModelMetadata (+29 more)
...
Cohesion: 0.13
Nodes (21): AlertsPage(), NavItem, TacticalThreatToast(), SuspectTrajectoryModal(), SuspectTrajectoryModalProps, CLASS_COLORS, DetectionCanvas(), DetectionCanvasProps (+13 more)
...
Cohesion: 0.20
Nodes (15): batch_inference(), create_preprocessor(), get_model_input_size(), image_inference(), inference_status(), get, post, UploadFile (+7 more)
...
Cohesion: 0.15
Nodes (19): RecentAlerts(), AlertListener, alertsStore, ConnectionListener, connectionListeners, getAlertsWsUrl(), INCIDENT_STORAGE_KEY, initWebSocket() (+11 more)
...
Cohesion: 0.11
Nodes (20): Detection, Postprocessor, PostprocessorError, Any, Exception, ndarray, Decode output tensors into a list of detection lists (one list per frame in the…, Decode single-frame YOLOv8/v9/v11 output [4 + C, N]. (+12 more)
...
### Community 33 - "TrackerSessionStore"
Cohesion: 0.15
Nodes (6): Record that a frame was processed for ``camera_id``., Reset and remove the tracker session for ``camera_id``. Returns: True if the…, Reset and remove all tracker sessions. Returns: Number of sessions that were…, Return a JSON-serialisable summary of all active sessions., Thread-safe registry mapping ``camera_id`` → ``ByteTrackerWrapper``. Typical…, TrackerSessionStore
...
### Community 45 - "TrackedObject"
Cohesion: 0.18
Nodes (8): Any, Data models for tracking output. TrackedObject extends a raw Detection with a…, Snapshot of a tracked object within a single video frame. Attributes: track_id:…, Return a JSON-serialisable representation., TrackedObject, Any, Feed one frame's detections into ByteTracker and return tracked objects. Args:…, TrackedObject
...
Cohesion: 0.11
Nodes (28): Run bounded inference on frames from an uploaded video using ONNX batching (2…, video_inference(), _decode_image_upload(), fuse_dual_spectrum_images(), get_available_palettes(), get_spot_temperature(), PaletteInfo, Any (+20 more)
...
### Community 77 - "api"
Cohesion: 0.19
Nodes (10): MetricCard(), MetricCardProps, MetricsRow(), SystemStatus(), ConnectionStatus(), api, BackendStatusState, useBackendStatus() (+2 more)

### Community 78 - "lucide-react"
Cohesion: 0.13
Nodes (4): GisMapPage(), CameraFeedStrip(), cameras, lucide-react
...
Cohesion: 0.18
Nodes (11): AnprPage(), CamerasPage(), GeofencesPage(), CameraCard(), CameraCardProps, CameraGrid(), useCameras(), AnprRecord (+3 more)
...
- **Why does `Postprocessor` connect `Postprocessor` to `thermal.py`, `ResourceManager`, `inference_service.py`?**
